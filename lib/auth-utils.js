...
async function redeemSsoToken(req) {
  let ssoToken = req.get(req.app.get('openam-cookie-header')) || req.cookies[req.app.get('openam-cookie')];
      ...
      request.post(
        ...
        (err, proxyResponse, proxyResponseBody) => {
          if (err) {
            ...
            if (proxyResponse.statusCode >= '200' && proxyResponse.statusCode <= '299') {
